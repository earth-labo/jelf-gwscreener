...
def generate_pdf_report(results: Dict[str, Any]) -> bytes:
    ...
    story.append(Paragraph(f"診断日時: {report_date}", styles['Normal']))
    story.append(Spacer(1, 0.5*cm))
    
    # コンテンツタイプ
    content_type = results.get('content_type', '不明')
    story.append(Paragraph(f"コンテンツタイプ: {content_type}", styles['Normal']))
    ...
    story.append(Paragraph(f"<b>評価:</b> {description}", styles['Normal']))
    story.append(Spacer(1, 1*cm))
    ...
    if violations:
        ...
        for i, violation in enumerate(violations, 1):
            ...
            story.append(Paragraph(f"<b>{v_title}</b>", styles['Normal']))
        ...
        story.append(Paragraph("問題は検出されませんでした。", styles['Normal']))
        story.append(Spacer(1, 1*cm))
    ...
    if recommendations:
        ...
        for i, rec in enumerate(recommendations, 1):
            ...
            story.append(Paragraph(f"<b>{r_title}</b>", styles['Normal']))
            story.append(Spacer(1, 0.3*cm))
            
            # 現在の表現
            current = rec.get('current_expression', '')
            story.append(Paragraph(f"<b>現在の表現:</b> 「{current[:150]}」", styles['Normal']))
            story.append(Spacer(1, 0.2*cm))
            
            # 推奨する表現
            recommended = rec.get('recommended_expression', '')
            story.append(Paragraph(f"<b>推奨する表現:</b> 「{recommended[:150]}」", styles['Normal']))
            story.append(Spacer(1, 0.2*cm))
            
            # 理由
            explanation = rec.get('explanation', '')
            story.append(Paragraph(f"<b>理由:</b> {explanation[:200]}", styles['Normal']))
            story.append(Spacer(1, 0.7*cm))
    else:
        story.append(Paragraph("是正提案", heading_style))
        story.append(Paragraph("是正の必要はありません。", styles['Normal']))
        story.append(Spacer(1, 1*cm))
