...
def handle_video_analysis(api_key, model_key, system_prompt, criteria_sections,
    ...
    """動画診断の処理"""
    st.markdown("### 🎬 動画診断")
    st.markdown("診断したい動画をアップロードするか、YouTube URLを入力してください。")
    
    # タブで切り替え
    tab1, tab2 = st.tabs(["📁 ファイルアップロード", "🎥 YouTube URL"])
    ...
    with tab1:
        ...
        if uploaded_file:
            ...
            video_source = f"動画ファイル: {uploaded_file.name}"
    ...
    with tab2:
        youtube_url = st.text_input(
            ...
            help="YouTube動画のURLを入力してください（最長60秒まで処理）"
        )
        
        if youtube_url:
            if 'youtube.com/watch' in youtube_url or 'youtu.be/' in youtube_url:
                st.info("✅ YouTube URLが検出されました")
                video_source = f"YouTube: {youtube_url}"
                
                # YouTube動画をダウンロード
                with st.spinner("YouTube動画をダウンロード中..."):
                    try:
                        from modules.video_analyzer import download_youtube_video
                        video_data = download_youtube_video(youtube_url)
                        if video_data:
                            st.success(f"✅ ダウンロード完了（{len(video_data) / (1024*1024):.1f} MB）")
                        else:
